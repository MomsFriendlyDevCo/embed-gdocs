  ...
  */ let setHtml = (el, html)=>el.setHTML ? el.setHTML(html) : el.innerHTML = html;
    ...
    return fetch(settings.url, settings.urlOptions).then((res)=>res.text()).then((html)=>{
        ...
        if (settings.fixImageTitleAsLink) {
            var _doc$querySelectorAll2;
            _doc$querySelectorAll2 = doc.querySelectorAll("img[title]"), Array.from(_doc$querySelectorAll2).filter((el)=>/^https?:\/\//.test(el.getAttribute("title"))).forEach((el)=>{
